...
def reproduce(mother, father):
    ...
    '''
    child = mother.copy()
    crossover_point = random.randint(0,len(mother))
...
def mutate(individual):
    ...
    mutation[mutation_point] = random.randint(1,8)
...
def random_selection(population, fitness_fn):
    ...
    for e in ordered_population:
        fitness_population.append((e,fitness_fn(e)))
...
def fitness_function(individual):
    ...
    for i in range (0,2+1):
        if individual[i] == 1 and i == 0:
            fitness += 4
        if individual[i] == 1 and i == 1:
            fitness += 2
        if individual[i] == 1 and i == 2:
            fitness += 1
